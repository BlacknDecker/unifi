...
[If_approx, nval] = trapcomp_val(10^7, 0, 1, fun);
print_res('Trapezi Composita', nval, abs(If-If_approx))

[If_approx, nval] = simpcomp_val(2*10^6, 0, 1, fun);
print_res('Simpson Composita', nval, abs(If-If_approx))

[If_approx, nval] = trapad_val(0, 1, fun, tol);
print_res('Trapezi Adattativa', nval, abs(If-If_approx))

[If_approx, nval] = simpad_val(0, 1, fun, tol);
print_res('Simpson Adattativa', nval, abs(If-If_approx))

% il calcolo delle valutazioni di fun in realta non serve
% ci sono sempre n+1 valutazioni
...
% anche in questo caso il calcolo delle valutazioni di fun non serve
...
% utility per evitaredi ripetere codice
